...
public class CS400Graph<T> implements GraphADT<T> {
  ...
  protected class Path implements Comparable<Path> {
    ...
    public Path(Path copyPath, Edge extendBy) {
      ...
      for (T current : copyPath.dataSequence) {
        dataSequence.add(current);
      }
  ...
  protected Path dijkstrasShortestPath(T start, T end) {
    if (!vertices.containsKey(start) || !vertices.containsKey(end)) {
      throw new NoSuchElementException("Cannot find a valid path");// throw NoSuchElementException
                                                                   // if parameters are not in the
                                                                   // graph
    }
    Vertex startVertex = vertices.get(start);// convert parameters to vertex
    Vertex endVertex = vertices.get(end);
    Path startPath = new Path(startVertex);
    LinkedList<Vertex> visited = new LinkedList<Vertex>();// List to save the visited vertex
    PriorityQueue<Path> queue = new PriorityQueue<Path>();
    queue.add(startPath);


    while (!queue.peek().end.equals(endVertex)) {// traverse the whole graph until reach the end
                                                 // point
      Path currentPath = queue.poll();
      visited.add(currentPath.end);
      for (Edge current : currentPath.end.edgesLeaving) {
        if (!visited.contains(current.target)) { // only check the unvisited vertex
          queue.add(new Path(currentPath, current));
        }
      }
      if (queue.peek() == null) {// throw NoSuchElementException if cannot find path
        throw new NoSuchElementException("Cannot find a valid path");
      }
    }
    return queue.poll();
  }
